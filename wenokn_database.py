...
def load_gages_in_states(state_names: list[str]) -> gpd.GeoDataFrame:
    """
    Build a SPARQL query to find all gages inside the given states.
    Optimized by filtering states first before spatial operations.
    """
    if not state_names:
...
If the user's question is to find all dams on a river (for example, Find all dams on the Scioto River), 
return the following code:
    river_name = "Scioto River"
    river_gdf = load_river_by_name(river_name)
...
return the following code:
    ...
    river_buffer = river_gdf.geometry.buffer(0.001).unary_union
